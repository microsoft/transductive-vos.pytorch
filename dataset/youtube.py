...
class YouTubeTrain(datasets.ImageFolder):
    def __init__(self,
                 img_root,
                 annotation_root,
                 cropping=256,
                 ...
                 color_jitter=False):
        super(YouTubeTrain, self).__init__(img_root,
                                         transform=transform,
    ...
    def __getitem__(self, index):
        ...
        for i in range(self.frame_num):
            ...
            annotation = f.resize(annotation, self.img_size)
...
class YouTubeInference(datasets.ImageFolder):
    """
    ...
    def __init__(self,
                 ...
                 transform=None,
                 target_transform=None):
        super(YouTubeInference, self).__init__(root,
                                             transform=transform,
                                             target_transform=target_transform)
        self.img_bytes = []
        self.rgb_normalize = transforms.Compose([transforms.Resize(480),
                                                 transforms.ToTensor(),
                                                 transforms.Normalize(
                                                     mean=[0.485, 0.456, 0.406],
                                                     std=[0.229, 0.224, 0.225])])
    ...
    def __getitem__(self, index):
        ...
        img_number = int(path.split('.')[0][-4:])
        img = Image.open(BytesIO(self.img_bytes[index]))
        img = img.convert('RGB')

        img_original = np.asarray(img)

        output = self.rgb_normalize(img)
        return output, video_index, img_original, img_number
